...
(kommun, stat, lÃƒÂ¤nsstyrelse, etc.) appear together more often than expected 
by chance.

Key Features:
- Chi-square tests for independence
- Effect sizes (CramÃƒÂ©r's V)
...
ACTOR_SEED_TERMS = {
    ...
    'lÃƒÂ¤nsstyrelse': ['lÃƒÂ¤nsstyrelse', 'lÃƒÂ¤nsstyrelsen', 'lÃƒÂ¤nsstyrelser'],
    'region': ['region', 'regionen', 'regional', 'regionala'],
    'nÃƒÂ¤ringsliv': ['nÃƒÂ¤ringsliv', 'nÃƒÂ¤ringslivet', 'fÃƒÂ¶retag', 'fÃƒÂ¶retagen'],
    'civilsamhÃƒÂ¤lle': ['civilsamhÃƒÂ¤lle', 'civilsamhÃƒÂ¤llet', 'frivillig', 'frivilliga'],
    'fÃƒÂ¶rening': ['fÃƒÂ¶rening', 'fÃƒÂ¶reningen', 'fÃƒÂ¶reningar']
}
...
def cramers_v(chi2: float, n: int, r: int, c: int) -> float:
    """
    Calculate CramÃƒÂ©r's V effect size for chi-square test.
    
    CramÃƒÂ©r's V measures the strength of association between two categorical 
...
def interpret_cramers_v(v: float) -> str:
    """Provide interpretation of CramÃƒÂ©r's V effect size."""
    if v < 0.10:
        return "negligible"
    elif v < 0.30:
        return "weak"
...
class SentenceDataProcessor:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        pd.DataFrame with columns:
            ...
            - global_sentence_id: Globally unique sentence identifier
        ...
        # Ensure we have a globally unique sentence identifier
        if 'global_sentence_id' not in self.df.columns:
            print("WARNING: 'global_sentence_id' not found. Creating from (doc_id, sentence_id)...")
            self.df['global_sentence_id'] = self.df.groupby(['doc_id', 'sentence_id']).ngroup()
        
        # Set which column to use for unique sentence identification
        self.sentence_id_col = 'global_sentence_id'
        
        print(f"Loaded {len(self.df):,} category-sentence pairs")
        print(f"Unique sentences: {self.df[self.sentence_id_col].nunique():,}")
    ...
    def identify_actors(self, sentence_text: str) -> List[str]:
        ...
        ['kommun']
        >>> identify_actors("stat lÃƒÂ¤nsstyrelse samverkan")
        ['stat', 'lÃƒÂ¤nsstyrelse']
        """
        actors_found = []
    ...
    def create_binary_indicators(self) -> pd.DataFrame:
        ...
        sentence_cols = ['doc_id', 'municipality', 'year', 'maskad', 
                        self.sentence_id_col, 'sentence_text', 'word_count']
        df_sentences = self.df[sentence_cols].drop_duplicates(subset=[self.sentence_id_col])
        ...
        for category in EFFECT_CATEGORIES:
            ...
            sentences_with_cat = set(
                self.df[self.df['category'] == category][self.sentence_id_col]
            )
            df_sentences[indicator_col] = df_sentences[self.sentence_id_col].isin(
                sentences_with_cat
            ).astype(int)
...
class CooccurrenceAnalyzer:
    ...
    def test_pair(
        ...
        if contingency.shape[0] < 2 or contingency.shape[1] < 2:
            logging.warning(
                f"Skipping {concept1} Ã— {concept2}: degenerate table "
    ...
    def test_actor_cooccurrence(self) -> pd.DataFrame:
        ...
        (e.g., kommun and länsstyrelse appearing in same sentences).
...
class TemporalAnalyzer:
    ...
    def test_concept_over_time(
        ...
        df_temporal['year_numeric'] = pd.to_numeric(df_temporal['year'], errors='coerce')
        df_temporal = df_temporal[df_temporal['year_numeric'].notna()]

        for start, end in periods:
            mask = (df_temporal['year_numeric'] >= start) & (df_temporal['year_numeric'] <= end)
            subset = df_temporal[mask]
            if len(subset) == 0:
                continue
    ...
    # Get data for each period
        mask1 = (df_temporal['year_numeric'] >= period1[0]) & (df_temporal['year_numeric'] <= period1[1])
        mask2 = (df_temporal['year_numeric'] >= period2[0]) & (df_temporal['year_numeric'] <= period2[1])
    
        subset1 = df_temporal[mask1]
        subset2 = df_temporal[mask2]
...
class ResultsVisualizer:
    ...
    def plot_cooccurrence_heatmap(
        ...
        """
        Plot heatmap of chi-square values or CramÃƒÂ©r's V.
...
def main():
    ...
    visualizer.plot_cooccurrence_heatmap(df_effect_cooc, 'effect-effect')
