...
(kommun, stat, länsstyrelse, etc.) appear together more often than expected 
by chance.
...
EFFECT_CATEGORIES = ['resilience', 'risk', 'complexity', 'efficiency', 'equality']
...
def cramers_v(chi2: float, n: int, r: int, c: int) -> float:
    ...
    Cramér's V measures the strength of association between two categorical 
...
def interpret_cramers_v(v: float) -> str:
    """Provide interpretation of Cramér's V effect size."""
    if v < 0.10:
        return "negligible"
    elif v < 0.30:
        return "weak"
...
class SentenceDataProcessor:
    ...
    def identify_actors(self, sentence_text: str) -> List[str]:
        ...
        ['kommun']
        >>> identify_actors("stat länsstyrelse samverkan")
        ['stat', 'länsstyrelse']
        """
        actors_found = []
...
class ResultsVisualizer:
    ...
    def plot_cooccurrence_heatmap(
        ...
        """
        Plot heatmap of chi-square values or Cramér's V.
