...
Creates a reproducible stratified sample of sentences from the RSA corpus
for hand-coding theoretical mechanisms (legitimacy, functional aptness,
spaces of equivalence, complexity empowerment).

Implements two-stage stratified sampling:
    1. Stage 1: Sample documents stratified by (actor_type, wave)
    2. Stage 2: Sample sentences within selected documents

Train/test split is performed at document level to prevent data leakage.

Input:
    - Sentence-level parquet (preferred): Pre-segmented sentences
...
Usage:
    ...
    python stratified_sample.py \\
        --input data/processed/sentences_lemmatized.parquet \\
        --output results/sampling/ \\
        --n-sentences 500 \\
        --seed 42 \\
        --verbose
...
def compute_allocation(
    ...
    -------
    pd.DataFrame
        Allocation table with columns: actor_type, wave, n_docs, n_sentences_target.
    """
    ...
    doc_counts['n_sentences_target'] = (doc_counts['proportion'] * n_sentences).astype(int)

    # Adjust for rounding (distribute remainder randomly)
    remainder = n_sentences - doc_counts['n_sentences_target'].sum()
    if remainder > 0:
        indices = rng.choice(len(doc_counts), size=remainder, replace=False)
        doc_counts.loc[doc_counts.index[indices], 'n_sentences_target'] += 1

    # Estimate documents needed per stratum
    doc_counts['n_docs_needed'] = np.ceil(
        doc_counts['n_sentences_target'] / sentences_per_doc
    ).astype(int)
...
def sample_sentences(
    ...
    """
    Stage 2: Sample sentences within selected documents.
    ...
    for _, alloc_row in allocation.iterrows():
        ...
        wave = alloc_row['wave']
        target = alloc_row['n_sentences_target']
        ...
        # Sample sentences from each document
        stratum_sentences = []
        for doc_file in stratum_docs:
            doc_sents = df[df['file'] == doc_file].copy()
            if len(doc_sents) == 0:
                continue

            n_sample = min(sentences_per_doc, len(doc_sents))
            selected_idx = rng.choice(len(doc_sents), size=n_sample, replace=False)
            stratum_sentences.append(doc_sents.iloc[selected_idx])

        if not stratum_sentences:
            continue

        stratum_df = pd.concat(stratum_sentences, ignore_index=True)
        ...
        sampled_sentences.append(stratum_df)

    if not sampled_sentences:
        raise ValueError("No sentences sampled. Check input data.")

    return pd.concat(sampled_sentences, ignore_index=True)
...
def format_output(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ----------
    df : pd.DataFrame
        Sampled sentences with split column.
...
def generate_report(
    ...
    report = {
        ...
        'sample_statistics': {
            ...
            'train_sentences': len(df[df['split'] == 'train']),
            'test_sentences': len(df[df['split'] == 'test']),
    ...
    if missing_actors:
        warnings.append(
            ...
            "Consider regenerating sentence corpus with all actors."
        )

    if len(df) < args.n_sentences:
        warnings.append(
            f"Could only sample {len(df)} sentences (requested {args.n_sentences})."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Create stratified sample of RSA sentences for hand-coding',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Sample from merged corpus (segments sentences on-the-fly)
    python stratified_sample.py \\
        --input data/merged/pdf_texts_all_actors.parquet \\
        --output results/sampling/ \\
        --n-sentences 500 --seed 42

    # Sample from pre-segmented sentences
    python stratified_sample.py \\
        --input data/processed/sentences_lemmatized.parquet \\
        --output results/sampling/ \\
        --n-sentences 500 --seed 42 --verbose
"""
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help='Total sentences to sample (default: 500)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Maximum sentences per document (default: 10)'
    )
    ...
    for actor in sorted(df['actor_type'].unique()):
        n_sent = len(df[df['actor_type'] == actor])
        n_doc = df[df['actor_type'] == actor]['file'].nunique()
        print(f"    {translate_actor(actor)}: {n_sent} sentences, {n_doc} documents")
    print(f"\n  By wave:")
    wave_labels = {0: 'pre-2015', 1: '2015-2018', 2: '2019-2022', 3: '≥2023'}
    for wave in sorted(df['wave'].unique()):
        n_sent = len(df[df['wave'] == wave])
        print(f"    Wave {wave} ({wave_labels.get(wave, '?')}): {n_sent} sentences")
    ...
    # Sample sentences
    print(f"\n{'='*60}")
    print("STAGE 3: Sampling sentences within documents")
    print(f"{'='*60}")
    ...
    print(f"\nTrain: {n_train} sentences ({n_train/len(sampled):.1%})")
    print(f"Test:  {n_test} sentences ({n_test/len(sampled):.1%})")

    # Format output
    print(f"\n{'='*60}")
    print("STAGE 5: Formatting output")
    print(f"{'='*60}")
    output_df = format_output(sampled)
    ...
    report = generate_report(output_df, allocation, args.input, args.output, args)
    ...
    print(f"\nSample size: {len(output_df)} sentences from {n_docs} documents")
    print(f"Train/test split: {n_train}/{n_test} ({args.train_ratio:.0%}/{1-args.train_ratio:.0%})")
    print(f"\nOutput directory: {args.output}")
    print(f"\nFiles for hand-coding:")
    print(f"  - sample_train.csv: {n_train} sentences (for training BERT)")
    print(f"  - sample_test.csv: {n_test} sentences (for evaluation)")
