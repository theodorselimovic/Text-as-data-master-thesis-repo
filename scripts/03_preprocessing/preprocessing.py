...
Input Format (from R readtext):
    RDS file with columns:
...
Usage:
    ...
    python readingtexts.py --input readtext_success.rds --output output.parquet --stopwords custom_stopwords.txt
...
class DocumentMetadataParser:
    ...
    def parse_filename(filename: str) -> Dict[str, any]:
        ...
        >>> parse_filename("RSA Gnosjö 2019.pdf")
        {'municipality': 'Gnosjö', 'year': '2019', 'maskad': False}
        """
...
class CorpusProcessor:
    ...
    def load_rds_file(self) -> pd.DataFrame:
        """
        Load RDS file from R readtext.
        ...
        if 'file' not in df.columns or 'text' not in df.columns:
            raise ValueError(
                f"RDS file must contain 'file' and 'text' columns. "
...
def run_pipeline(
    ...
    logging.info("=" * 80)
    logging.info("\nNext steps:")
    logging.info("  1. ✓ Preprocessing complete")
    logging.info("  2. → Run vectoranalysis.py to expand seed terms")
    logging.info("  3. → Run sentencefiltering.py to create filtered dataset")
    logging.info("  4. → Run cooccurrence_analysis.py for statistical tests")
...
Examples:
    ...
    python readingtexts.py --input readtext_success.rds --output data/processed/sentences.parquet
    
    # With custom stopwords
    python readingtexts.py --input readtext_success.rds --stopwords custom_stopwords.txt
    
    # Keep stopwords (no removal)
    python readingtexts.py --input readtext_success.rds --no-remove-stopwords
    
    # Verbose output
    python readingtexts.py --input readtext_success.rds --verbose

Input Format:
    RDS file from R's readtext package with columns:
    - file: PDF filename (e.g., "RSA Ale 2015 Maskad.pdf")
    - text: Extracted text content
...
Stopword Removal:
    ...
    parser.add_argument(
        ...
        help=f'Input RDS file from R readtext (default: {DEFAULT_INPUT_FILE})'
    )
